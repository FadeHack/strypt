...
§9.1.1's 14-bit sync is the only confirmation the walk finished where the file claimed; and a
**FLAC with a prepended ID3v2 tag**, which is refused by name — reading that tag is the MP3
tranche's job, and cleaning the blocks around it would report success on a file strypt had not
finished.

**The audio is never decoded.** Anything in a frame's reserved bits, or appended past the last
frame, is out of reach — for mat2 and ExifTool as much as for strypt. Every FLAC report carries
this note, clean files included.

**Testing.** 10 well-formed fixtures and 8 malformed ones in `corpus/flac`, generated by
...
mat2 and ffmpeg can open them. 19 integration tests in `crates/strypt-core/tests/flac.rs`, checking
...
and byte-flip sweeps. 26 unit tests in the handler. Every malformed fixture is refused.
