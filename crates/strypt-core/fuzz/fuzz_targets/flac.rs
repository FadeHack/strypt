//! Fuzz target for the FLAC handler.
//!
...
//!
//! A FLAC is edited by block surgery (ADR-0038), so output larger than input means bytes were
//! synthesised. Padding is the one block written rather than copied, and it is written at the
//! length it already had, so the bound holds.
//!
//! The guard is not optional. These targets drive the whole pipeline, so a mutation is free to
//! land on another format's magic and be dispatched to that format's handler, where a FLAC-shaped
...
//!
//! Finding the frames in an arbitrary byte string means parsing the file, and a target that
//! re-implements the parser under test asserts only that two copies of the same logic agree. It
//! is checked over the corpus instead, in `tests/flac.rs`, by an independent block walker.
//!
...
fuzz_target!(|data: &[u8]| {
    ...
    if matches!(detect(data), Ok(Format::Flac)) {
        assert!(
            first.bytes.len() <= data.len(),
            "stripping a FLAC produced more bytes than it was given"
        );
    }
