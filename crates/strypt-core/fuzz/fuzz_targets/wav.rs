//! Fuzz target for the WAV handler.
//!
...
//!
//! A WAV is edited by chunk surgery (ADR-0039), so output larger than input means bytes were
//! synthesised. `JUNK` and `PAD ` are the only chunks written rather than copied, and they are
//! written at the length they already had, so the bound holds.
//!
//! The guard is not optional. These targets drive the whole pipeline, so a mutation is free to
//! land on another format's magic and be dispatched to that format's handler, where a WAV-shaped
...
//!
//! # Why "the audio is unchanged" is not asserted here
//!
//! Finding the `data` chunk in an arbitrary byte string means parsing the file, and a target that
//! re-implements the parser under test asserts only that two copies of the same logic agree. It
//! is checked over the corpus instead, in `tests/wav.rs`, by an independent chunk walker.
//!
...
fuzz_target!(|data: &[u8]| {
    ...
    if matches!(detect(data), Ok(Format::Wav)) {
        assert!(
            first.bytes.len() <= data.len(),
            "stripping a WAV produced more bytes than it was given"
        );
    }
