#!/usr/bin/env bash
# Differential test: strypt against mat2 and ExifTool over the OpenDocument corpus.
#
...
# mat2 rebuilds an OpenDocument package from a whitelist of parts it recognises — dropping
# settings.xml, Thumbnails/, Configurations2/ and ObjectReplacements/ outright — and strypt
# copies every part it did not have a reason to change. Comparing bytes would report a difference
# on every file and tell you nothing. What is compared is *what metadata survives in each tool's
# output*.
#
...
for input in "$CORPUS"/*.odt "$CORPUS"/*.ods "$CORPUS"/*.odp; do
    ...
    if ! (cd "$WORK/m" && mat2 --inplace "$name" >/dev/null 2>&1); then
        note "mat2 refuses this file — recorded, not skipped: see docs/THREAT_MODEL.md §7.7"
        continue
    fi
    ...
    # never because it was inconvenient. Both are the same exclusions the OOXML differential
    ...
    # else "Weird", so it reports strypt's constant 0 as a finding.
    ...
    if [ "$survived_strypt" -gt "$survived_mat2" ]; then
        ...
        grep -vE "$exclude" "$WORK/strypt.txt" | grep -E '^\s+\S+:' | sed 's/^/      /'
        gaps=$((gaps + 1))
    else
        note "strypt: $survived_strypt field(s) survive · mat2: $survived_mat2 — no gap"
    fi

    # ExifTool is the second opinion, and the one that reads the embedded pictures. mat2's ODF
    # reader lists part metadata; ExifTool walks into Pictures/.
