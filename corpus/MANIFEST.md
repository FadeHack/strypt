...
| `malformed/duplicate-kids.pdf` | A page tree that is itself a `/Page` and lists one object three times | **A regression test for a real bug.** lopdf permutes page objects before renumbering, so two ids mapped onto one and an object was dropped — here the document's only page. 0.1.1 wrote that file and reported success. Now refused |
